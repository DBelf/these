...
public class ASTConstructor extends ECMAScriptBaseVisitor<ASTNode>{
    ...
    public ASTNode visitVariableStatement(ECMAScriptParser.VariableStatementContext ctx) {
        ...
        for (ECMAScriptParser.VariableDeclarationContext variableDeclarationContext : variableDeclarationListContext.variableDeclaration()){
            statements.add((Statement) variableDeclarationContext.accept(this));
        }
        return statements;
    }

    @Override
    public ASTNode visitVariableDeclaration(ECMAScriptParser.VariableDeclarationContext ctx) {
        Identifier identifier = new Identifier(ctx.Identifier().getText());
        Statement expression = (Statement) ctx.initialiser().accept(this);
    ...
    @Override
    public ASTNode visitForStatement(ECMAScriptParser.ForStatementContext ctx) {
        Statements forHeader = new Statements();
        for (ECMAScriptParser.ExpressionSequenceContext expression : ctx.expressionSequence()) {
            forHeader.add((Statement) expression.accept(this));
        }

        Statement forBody = (Statement) ctx.statement().accept(this);
        return new ForStatement(forHeader, forBody);
    }
